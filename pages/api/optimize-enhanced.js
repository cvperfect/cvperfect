...
import { validateUsageBeforeOptimization } from '../../utils/usageValidation'
...
// Helper function: Email validation and sanitization
...
export default async function handler(req, res) {
  ...
  try {
    const { documentType, content, jobPosting, jobPostingData, targetRegion = 'PL', userId, email } = req.body
    ...
    if (jobPostingData?.text && jobPostingData.text.length > MAX_JOB_POSTING_LENGTH) {
      return res.status(400).json({
        success: false,
        error: `Oferta pracy zbyt długa. Maksymalna długość: ${MAX_JOB_POSTING_LENGTH} znaków`
      });
    ...
    // Story 4.4: Payment Gate - Require payment for all optimization (no demo mode)
    if (!email || email === '') {
      console.log('❌ Enhanced optimization - Missing email - payment required');
      return res.status(403).json({
        success: false,
        error: 'Musisz wykupić plan aby korzystać z optymalizacji CV. Przejdź do strony z cennikiem.',
        code: 'PAYMENT_REQUIRED',
        redirectTo: '/pricing'
      });
    }

    // Validation - Email format and sanitization
    ...
    console.log('🔍 Enhanced optimization - checking user payment status for:', sanitizedEmail);

    // No demo mode - all users must have valid payment
    const validation = await validateUsageBeforeOptimization(sanitizedEmail, false);

    if (!validation.success) {
      console.log('❌ Enhanced optimization - payment validation failed:', validation.code);
      return res.status(403).json({
        success: false,
        error: validation.error + ' Przejdź do strony z cennikiem aby wykupić plan.',
    ...
    const processingResult = await Promise.race([
      processOptimization(content, jobPosting, jobPostingData, targetRegion, userId, startTime),
      timeoutPromise
    ])
...
async function processOptimization(content, jobPosting, jobPostingData, targetRegion, userId, startTime) {
...
4. Zachowaj czytelną strukturę z sekcjami: Dane osobowe, Podsumowanie zawodowe, Doświadczenie, Wykształcenie, Umiejętności, Certyfikaty`
    ...
    // Build user prompt with enhanced job posting integration
    let userPrompt = `ORYGINALNE CV DO OPTYMALIZACJI:\n${content}\n\n`

    // Use structured job posting data if available, otherwise fall back to simple text
    const jobData = jobPostingData || { text: jobPosting };

    if (jobData && (jobData.text || jobData.url)) {
      userPrompt += `OFERTA PRACY - INFORMACJE DO DOPASOWANIA:\n`;

      if (jobData.validation && jobData.validation.extractedKeywords && jobData.validation.extractedKeywords.length > 0) {
        userPrompt += `KLUCZOWE UMIEJĘTNOŚCI: ${jobData.validation.extractedKeywords.join(', ')}\n`;
      }

      if (jobData.text) {
        // Sanitize job posting to prevent prompt injection
        const sanitizedJobText = sanitizeJobPosting(jobData.text);
        userPrompt += `TREŚĆ OFERTY: ${sanitizedJobText}\n`;
      } else if (jobData.url) {
        userPrompt += `ŹRÓDŁO: ${jobData.url}\n`;
      }

      userPrompt += `\nDOPASOWANIE: Naturalnie wpleć powyższe słowa kluczowe i dopasuj CV do wymagań tej oferty.\n\n`;
    }
    ...
    // Calculate metrics
    const keywordCoverage = (jobData && (jobData.text || jobData.validation?.extractedKeywords)) ?
      calculateKeywordCoverage(optimizedContent, jobData) : 'N/A'
    
    const metrics = {
      atsScore: calculateATSScore(optimizedContent, jobData),
    ...
    try {
      ...
      const historyRecord = {
        ...
        job_posting: jobData?.text ? safeSubstring(jobData.text, 5000) : (jobPosting ? safeSubstring(jobPosting, 5000) : null),
        job_posting_data: jobData ? JSON.stringify(jobData) : null,
...
function calculateATSScore(content, jobData) {
  let score = 70 // Base score for enhanced optimization

  if (jobData) {
    // Use structured keywords if available
    if (jobData.validation?.extractedKeywords?.length > 0) {
      const keywordMatches = jobData.validation.extractedKeywords.filter(keyword =>
        content.toLowerCase().includes(keyword.toLowerCase())
      ).length;

      score += Math.min(keywordMatches * 3, 20); // Max 20 points for keyword matches
    } else if (jobData.text) {
      // Fallback to simple text matching
      score += 15
    }

    // Bonus for URL-sourced job postings (higher quality)
    if (jobData.url && jobData.validation?.contentQuality > 60) {
      score += 5;
    }
  }
...
function calculateKeywordCoverage(content, jobData) {
  if (!jobData) return 'N/A'

  let jobKeywords = [];

  // Use structured keywords if available (more accurate)
  if (jobData.validation?.extractedKeywords?.length > 0) {
    jobKeywords = jobData.validation.extractedKeywords;
  } else if (jobData.text) {
    // Fallback to text parsing
    jobKeywords = jobData.text.toLowerCase()
      .split(/\s+/)
