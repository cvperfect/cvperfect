...
function authenticateAdmin(req, res) {
  const adminKey = process.env.PERFORMANCE_DASHBOARD_KEY || "cvp_admin_2025_secure_key_xyz789"

  // For development, use the hardcoded key if env var not set
  if (!adminKey) {
  ...
  if (!providedKey || providedKey !== adminKey) {
    console.warn("Unauthorized API access attempt to performance-metrics", {
    ...
    res.status(404).json({ error: "Not found" }) // Return 404 instead of 403 for security
    return false
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (metric_name === "LCP" && actualValue > 4000) {
      console.warn(`🚨 Critical LCP: ${actualValue}ms (target: ≤2500ms) for ${url}`);
    }
    
    if (metric_name === "CLS" && actualValue > 0.25) {
      console.warn(`🚨 Critical CLS: ${actualValue} (target: ≤0.1) for ${url}`);
    }

    if (metric_name === "FID" && actualValue > 300) {
      console.warn(`🚨 Critical FID: ${actualValue}ms (target: ≤100ms) for ${url}`);
    }
    
    if (metric_name === "FCP" && actualValue > 3000) {
      console.warn(`🚨 Poor FCP: ${actualValue}ms (target: ≤1800ms) for ${url}`);
    }
    
    if (metric_name === "TTFB" && actualValue > 1500) {
      console.warn(`🚨 Poor TTFB: ${actualValue}ms (target: ≤600ms) for ${url}`);
    }
    
    // API performance alerts
    if (metric_name === "API" && metric_data?.duration > 1000) {
      console.warn(`🚨 Slow API: ${metric_data.endpoint} took ${metric_data.duration}ms for ${url}`);
    }
    
    // Bundle performance alerts
    if (metric_name === "BUNDLE_BUDGET_EXCEEDED") {
      const sizeKB = Math.round(actualValue / 1024);
      console.warn(`🚨 Bundle budget exceeded: ${sizeKB}KB (target: 293KB) for ${url}`);
    }
    
    if (metric_name === "BUNDLE" && actualValue > 3000) {
      console.warn(`🚨 Slow bundle loading: ${actualValue}ms for ${url}`);
    }
    ...
    if (metric_name === "CUSTOM_METRICS" && typeof metric_data === "object") {
      ...
      if (customData.memoryUsage?.usedJSMemory > 100000000) { // 100MB
        ...
        console.warn(`🚨 High memory usage: ${memoryMB}MB for ${url}`);
      }
      
      // Load time alerts
      if (customData.pageLoadTime > 5000) { // 5 seconds
        console.warn(`🚨 Slow page load: ${customData.pageLoadTime}ms for ${url}`);
      }
    }
    
    // Long task alerts
    if (metric_name === "LONG_TASK" && actualValue > 200) {
      console.warn(`🚨 Very long task detected: ${actualValue}ms (blocking main thread) for ${url}`);
    }
    
    // Critical alert notifications
    if (metric_name === "PERFORMANCE_CRITICAL_ALERT") {
      console.error(`🚨🚨 CRITICAL PERFORMANCE ALERT for ${url}:`, metric_data);
    }
