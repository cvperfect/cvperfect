...
class CvPerfectTester {
    ...
    async testSiteAccess() {
        console.log('\n🧪 TEST 1: Dostępność strony');
        try {
            const response = await this.page.goto('http://localhost:3010', { 
    ...
    async testDataFlow() {
        ...
        try {
            ...
            await this.page.goto('http://localhost:3010/success', { waitUntil: 'networkidle2' });
    ...
    async testAPIEndpoints() {
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                }, `http://localhost:3010${endpoint.url}`, endpoint.method);
